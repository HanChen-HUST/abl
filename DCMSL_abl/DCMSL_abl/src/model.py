...
class DCMSL(torch.nn.Module):
    ...
    def community_contrastive_with_cs(self, z1, z2, communities,delta,gamma,coms) -> torch.Tensor:
       ...
       # print(readout_z1.shape,coms.shape)
        ...
        pfgcl=-torch.log(between_sim_node.diag() / (refl_sim_node.sum(1) + between_sim_node.sum(1) - refl_sim_node.diag()))
        return torch.mean(pfgcl)+gamma*torch.mean(clgcl)
    def community_contrastive_with_cs2(self, z1_batch, z2_batch, z1, z2,nlb, communities, delta, gamma,coms) -> torch.Tensor:
        #print("ori")
       ...
       # print(readout_z1.shape,coms.shape)
        ...
        pfgcl=-torch.log(between_sim_node.diag() / (refl_sim_node.sum(1) + between_sim_node.sum(1) - refl_sim_node.diag()))
        return torch.mean(pfgcl)+gamma*torch.mean(clgcl)
    def rand_pool_community_contrastive(self, z1_batch, z2_batch, z1, z2,nlb, communities, delta, gamma) -> torch.Tensor:
        #print("rand")
        communities = torch.tensor(communities, device=z1.device)
        ...
        pfgcl=-torch.log(between_sim_node.diag() / (refl_sim_node.sum(1) + between_sim_node.sum(1) - refl_sim_node.diag()))
        return torch.mean(pfgcl)+gamma*torch.mean(clgcl)
    def rand_pool_community_contrastive2(self, z1_batch, z2_batch, z1, z2,nlb, communities, delta, gamma) -> torch.Tensor:
        #print("rand")
        communities = torch.tensor(communities, device=z1.device)
    ...
    def max_pool_community_contrastive2(self, z1_batch, z2_batch, z1, z2,nlb, communities, delta, gamma) -> torch.Tensor:
        #print("max")
        communities = torch.tensor(communities, device=z1.device)
        unique_communities, inverse_indices = torch.unique(communities, return_inverse=True)
        num_communities = unique_communities.size(0)
    ...
    def community_contrastive2(self, z1_batch, z2_batch, z1, z2,nlb, communities, delta, gamma) -> torch.Tensor:
        ...
        return torch.mean(pfgcl) + gamma * torch.mean(clgcl)
    def msgcl(self,z11,z22,z1,z2,nlb,com,delta,gamma,coms) -> torch.Tensor:
        ...
        if self.poolway=="max":
            l1 = self.max_pool_community_contrastive2(h1, h2,com,delta,gamma)
            l2 = self.max_pool_community_contrastive2(h2, h1,com,delta,gamma)
        if self.poolway=="rand":
            l1 = self.rand_pool_community_contrastive2(h1, h2,com,delta,gamma)
            l2 = self.rand_pool_community_contrastive2(h2, h1,com,delta,gamma)
        if self.poolway=="cs":
            l1 = self.community_contrastive_with_cs2(h1, h2,com,delta,gamma,coms)
            l2 = self.community_contrastive_with_cs2(h2, h1,com,delta,gamma,coms)
        if self.poolway=="infonce":
            l1 = self.batched_semi_loss(h1, h2, 40960)
            l2 = self.batched_semi_loss(h2, h1, 40960)
